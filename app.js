...
class App {
  ...
  registerCustomItems(items) {
    ...
    items.forEach(item => {
      ...
      const normalized = {
        ...
        slot: item.slot || "item-slot-custom",
        isCustom: true
      };
  ...
  renderItemInventory() {
    ...
    this.getItemsForDev(activeDev?.id).forEach(item => {
  ...
  createCustomItemFromDrawing() {
    ...
    const item = {
      ...
      ownerId: this.currentSelectedDevId || null,
      slot: "item-slot-custom",
      isCustom: true
    };
  ...
  updatePokedexDetail(devId) {
    ...
    this.renderRestaurantMap(dev.id);

    const layerBox = document.getElementById("equipped-visuals-container");
    layerBox.innerHTML = "";
    const equippedItems = (dev.equipped || [])
      .map(itemId => window.ITEMS.find(item => item.id === itemId))
      .filter(item => item && (!item.ownerId || item.ownerId === dev.id));

    if (equippedItems.length > 0) {
      equippedItems.forEach(item => {
        const el = document.createElement("div");
        el.className = `equipped-layer ${item.id} ${item.slot || "item-slot-custom"}${item.isCustom ? " custom-equipped" : ""}`;
        if (item.imageData) {
          const img = document.createElement("img");
          img.src = item.imageData;
          img.alt = item.name;
  ...
  renderRestaurantMap(activeDevId) {
    ...
    restaurants.forEach(restaurant => {
      ...
      item.className = `restaurant-item${restaurant.memberId === activeDevId ? " active" : ""}`;
      item.href = this.getRestaurantSearchUrl(restaurant);
      item.target = "_blank";
      item.rel = "noopener noreferrer";

      const top = document.createElement("div");
      top.className = "restaurant-item-top";

      const member = document.createElement("span");
      member.textContent = restaurant.memberName;

      const category = document.createElement("em");
      category.textContent = restaurant.category;

      const name = document.createElement("strong");
      name.textContent = restaurant.placeName;
    ...
    const appKey = this.getKakaoMapAppKey();
    if (!appKey) {
  ...
  getKakaoMapAppKey() {
    const metaKey = document.querySelector('meta[name="kakao-map-app-key"]')?.content?.trim();
    return (window.KAKAO_MAP_APP_KEY || metaKey || "").trim();
  }
  ...
  loadKakaoMapSdk(appKey) {
    ...
    if (this.kakaoMapSdkPromise) {
      return this.kakaoMapSdkPromise;
    }

    this.kakaoMapSdkPromise = new Promise((resolve, reject) => {
  ...
  async drawRestaurantMap(restaurants, activeDevId) {
    ...
    results.forEach(({ restaurant, place }, index) => {
      ...
      const mapUrl = place?.id ? `https://place.map.kakao.com/${place.id}` : this.getRestaurantSearchUrl(restaurant);
      const infoWindow = new kakao.maps.InfoWindow({
        content: `
          <div class="kakao-restaurant-info">
            <strong>${index + 1}. ${this.escapeHtml(restaurant.placeName)}</strong>
            <span>${this.escapeHtml(restaurant.memberName)} · ${this.escapeHtml(restaurant.category)}</span>
            <a href="${mapUrl}" target="_blank" rel="noopener noreferrer">카카오맵에서 보기</a>
          </div>
        `
      });
      ...
      kakao.maps.event.addListener(marker, "click", () => {
        ...
        infoWindow.open(this.restaurantMap, marker);
      });

      this.restaurantMarkers.push(marker);
      this.restaurantInfoWindows.push(infoWindow);
      bounds.extend(position);

      if (restaurant.memberId === activeDevId) {
        infoWindow.open(this.restaurantMap, marker);
      }
    });
  ...
  clearRestaurantMap() {
    this.restaurantInfoWindows.forEach(infoWindow => infoWindow.close());
  ...
  getRestaurantSearchUrl(restaurant) {
    return `https://map.kakao.com/link/search/${encodeURIComponent(restaurant.query || restaurant.placeName)}`;
  }
